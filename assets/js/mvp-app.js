...
const db = {
    read: (key) => {
        ...
        } catch (error) { console.error(`db.read: Error parsing for key "${DB_PREFIX + key}":`, error); localStorage.removeItem(DB_PREFIX + key); return []; }
    },
    write: (key, data) => {
        try { localStorage.setItem(DB_PREFIX + key, JSON.stringify(data)); }
        catch (error) { console.error(`db.write: Error for key "${DB_PREFIX + key}":`, error); }
    },
...
function handleClientFormSubmit(e) {
    ...
    if(id){clients=clients.map(c=>c.id===id?{...c,...clientData}:c);}else{clientData.id=db.generateId();clients.push(clientData);}
...
function handleDeleteClient(id) {
    ...
    confirmModalConfirmBtnElem.onclick=()=>{clients=clients.filter(cl=>cl.id!==id);db.write('clients',clients);assignedWorkouts=assignedWorkouts.filter(aw=>aw.clientId!==id);db.write('assignedWorkouts',assignedWorkouts);renderClients();renderAssignedWorkouts();closeModal(confirmModal);};
...
function handleExerciseFormSubmit(e){
    ...
    if(id){exercises=exercises.map(ex=>ex.id===id?{...ex,...exData}:ex);}else{exData.id=db.generateId();exercises.push(exData);}
...
function handleDeleteExercise(id){
    ...
    const newBtn=confirmModalConfirmBtnElem.cloneNode(true);confirmModalConfirmBtnElem.parentNode.replaceChild(newBtn,confirmModalConfirmBtnElem);confirmModalConfirmBtnElem=newBtn;
    confirmModalConfirmBtnElem.onclick=()=>{exercises=exercises.filter(e=>e.id!==id);db.write('exercises',exercises);workoutTemplates=workoutTemplates.map(t=>({...t,exercises:t.exercises.filter(tei=>tei.exerciseId!==id)}));db.write('workoutTemplates',workoutTemplates);renderExercises();renderWorkoutTemplates();closeModal(confirmModal);};
    openModal(confirmModal);
}
...
function handleWorkoutTemplateFormSubmit(e){
    ...
    if(id){workoutTemplates=workoutTemplates.map(wt=>wt.id===id?{...wt,...tData}:wt);}else{tData.id=db.generateId();workoutTemplates.push(tData);}
...
function handleDeleteTemplate(id){
    ...
    const newBtn=confirmModalConfirmBtnElem.cloneNode(true);confirmModalConfirmBtnElem.parentNode.replaceChild(newBtn,confirmModalConfirmBtnElem);confirmModalConfirmBtnElem=newBtn;
    confirmModalConfirmBtnElem.onclick=()=>{workoutTemplates=workoutTemplates.filter(wt=>wt.id!==id);db.write('workoutTemplates',workoutTemplates);assignedWorkouts=assignedWorkouts.filter(aw=>aw.workoutTemplateId!==id);db.write('assignedWorkouts',assignedWorkouts);renderWorkoutTemplates();renderAssignedWorkouts();closeModal(confirmModal);};
    openModal(confirmModal);
}
...
function handleAssignWorkoutFormSubmit(e){
    ...
    const ass={id:db.generateId(),clientId:assignClientSelect.value,workoutTemplateId:assignTemplateSelect.value,dateAssigned:assignDateInput.value,status:'pending'};
    assignedWorkouts.push(ass);db.write('assignedWorkouts',assignedWorkouts);renderAssignedWorkouts();assignWorkoutForm.reset();
}
function handleDeleteAssignment(id){
    if(!confirm("Remove assignment?"))return;
    assignedWorkouts=assignedWorkouts.filter(aw=>aw.id!==id);db.write('assignedWorkouts',assignedWorkouts);renderAssignedWorkouts();
}
    ...
    if (mvpThemeSwitcherElement && bodyElement) {
        initializeThemeSwitcher(
            ...
            (newThemeValue, currentThemeObject) => {
                ...
                if (currentThemeObject) { // currentThemeObject is passed by initializeThemeSwitcher
                    ...
                    if(sidebarAvatarElem && currentThemeObject.sidebarUser) sidebarAvatarElem.textContent = currentThemeObject.sidebarUser.match(/\b(\w)/g)?.join('').substr(0,2).toUpperCase() || 'PT';
                }
            },
            'fitflowGlobalTheme' // Use global key
        );
    } else if (FITFLOW_THEMES_CONFIG.length > 0 && bodyElement) { // Fallback if switcher element not found but themes exist
        ...
        if (sidebarUserElem && defaultTheme.sidebarUser) sidebarUserElem.textContent = defaultTheme.sidebarUser;
        if (sidebarAvatarElem && defaultTheme.sidebarUser) sidebarAvatarElem.textContent = defaultTheme.sidebarUser.match(/\b(\w)/g)?.join('').substr(0,2).toUpperCase() || 'PT';
        [addEditClientModal, confirmModal, createEditTemplateModal, viewClientPlanModal].forEach(m => { if(m) m.classList.add(defaultTheme.value); });
    }
